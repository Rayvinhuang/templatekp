    ...
    <form class="forms-sample" method="POST" action="{{ route("customer.store")}}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select name="makanan_id" class="form-control">
                ...
                <option value="{{ $item->id }}"
                    @if (old('makanan_id', $pemesanan->makanan_id) == $item['id'])
                    selected
                    @endif
                    >{{ $item-> namaMembership }}</option>
                @endforeach
            </select>
            @error('makanan_id')
        ...
        <div class="form-group">
            ...
            <select name="minuman_id" class="form-control">
                ...
                <option value="{{ $item->id }}"
                    @if (old('minuman_id', $pemesanan->minuman_id) == $item['id'])
                    selected
                    @endif
                    >{{ $item-> namaMinuman }}</option>
                @endforeach
            </select>
            @error('minuman_id')
            <label class="text-danger">{{ $message }} </label>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="jumlah" placeholder="Jumlah" value="{{$pemesanan->jumlah}}">
            @error('jumlah')
                <label class="text-danger">{{ $message }} </label>
            @enderror
            </div>
